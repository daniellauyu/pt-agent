...
- 支持单个资源“一键下载”和批量“一键下载推荐”，下载统一提交 PT Core
- Core 在每次入队时重新执行安全准入，不信任插件自报分数
- Core 首次发送时自动创建并使用 qBittorrent 的 `PT_AGENT` 分类
- Core 自动写入 `ptagent` 标签，并以保留时区的 ISO 8601 标签记录 Free 截止时间
...
- 根据 Tracker 认可做种容量与当前魔力效率，显示建议总容量和还需新增容量区间；Core 在线时使用权威预测，离线时使用同结构的本地回退
- 新手考核卡会统计 qB 中 `queuedUP` 与实际活跃的 M-Team 做种任务，优先提示上传队列导致 Tracker 未认可做种的问题
- Free Guard 默认由 Core 唯一执行；扩展后台只提供只读预警
- Core 自动删除默认关闭；显式启用后仍会保护 `PT_AGENT_NODEL/KEEP` 标签任务
...
3. 启动 PT Core，在资源行点击“一键下载”，或点击顶部“一键下载推荐”。
...
插件重新加载后会注册每分钟一次的只读检查。Core 是默认且唯一的删除执行者；通过 `PT_AGENT_QB_AUTO_DELETE_ENABLED=true` 显式开启执行模式。Core 只处理同时满足以下条件的任务：
